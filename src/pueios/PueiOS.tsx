...
[style*="cursor: progress"], [style*="cursor:progress"] { cursor: ${arrowBusy} 3 2, progress !important; }`;
  ...
  const wallpaperStyle: React.CSSProperties = (() => {
    ...
    const namedWallpapers = ["bliss","pueisky","fencesunset","dutchvillage","greekcoast","milkyway","aero-blue","aero-pink","aero-neon","aero-dusk","aero-forest","aero-ember","aero-arctic","default"];
    if (theme.wallpaper === "puei") {
      return { backgroundImage: "url(/puei.jpg)", backgroundSize: "cover", backgroundPosition: "center" };
    }
    if (systemVersion === "PueiOS 4" && !namedWallpapers.includes(theme.wallpaper) && !theme.wallpaper.startsWith("data:") && !theme.wallpaper.startsWith("custom:")) {
      return { backgroundImage: "url(/puei.jpg)", backgroundSize: "cover", backgroundPosition: "center" };
    }
    return {};
  })();
  ...
  return (
    <div
      className={`fixed inset-0 ${isP3 ? "win7-aero" : ""} ${theme.wallpaper === "puei" || theme.wallpaper.startsWith("custom:") || theme.wallpaper.startsWith("data:") ? "" : `wallpaper-${theme.wallpaper}`}`}
    ...
    >
      ...
      {startOpen && systemVersion === "PueiOS 1" && (() => {
        const aC = theme.accentC ?? 0.22;
        const aH = theme.accentH;
        const isWhitePreset = aC < 0.05 && theme.accentL !== undefined && theme.accentL > 0.5;
        const isBlackPreset = aC < 0.05 && theme.accentL !== undefined && theme.accentL <= 0.5;
        const headerBg = isWhitePreset
          ? "linear-gradient(180deg, #888, #bbb)"
          : isBlackPreset
          ? "linear-gradient(180deg, #111, #333)"
          : `linear-gradient(180deg, oklch(0.30 ${aC} ${aH}), oklch(0.46 ${aC} ${aH}))`;
        const hoverBg = isWhitePreset ? "#999" : isBlackPreset ? "#222" : `oklch(0.36 ${aC} ${aH})`;
        const hoverColor = isWhitePreset ? "#fff" : "#fff";
        const accent = (l: number, c?: number) => `oklch(${l} ${c ?? aC} ${aH})`; void accent;
        return (
        <div className="fixed bottom-9 left-0 z-[9000]"
          ...
          <div style={{ background: headerBg, padding: "8px 10px", color: "#fff", fontWeight: "bold", fontSize: 14, display: "flex", alignItems: "center", gap: 8 }}>
            <PueiLogoSvg size={28} />
            <div>
      ...
      {taskViewOpen && isP4 && (
        <div className="fixed inset-0 z-[8500]"
          ...
          {windows.length === 0 ? (
            ...
            <div style={{ flex: 1, display: "flex", flexWrap: "wrap", alignContent: "flex-start", gap: 16, padding: "0 40px", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
              {windows.map((w) => {
                const title = APP_TITLES[w.appId] ?? w.appId;
                const isActive = !w.minimized && w.z === Math.max(...windows.map(x => x.z));
                return (
                  <div key={w.id}
                    onClick={(e) => { e.stopPropagation(); focusWin(w.id); setWindows(ws => ws.map(x => x.id === w.id ? { ...x, minimized: false } : x)); setTaskViewOpen(false); blip("click"); }}
                    style={{ width: 200, height: 130, background: "#1e1e1e", borderRadius: 8, overflow: "hidden", cursor: "pointer", position: "relative", border: `2px solid ${isActive ? "#0078d4" : "rgba(255,255,255,0.12)"}`, flexShrink: 0, transition: "transform 0.12s, border-color 0.12s, box-shadow 0.12s" }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform = "scale(1.05)"; (e.currentTarget as HTMLElement).style.borderColor = "#0078d4"; (e.currentTarget as HTMLElement).style.boxShadow = "0 8px 32px rgba(0,120,212,0.4)"; }}
                    onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform = ""; (e.currentTarget as HTMLElement).style.borderColor = isActive ? "#0078d4" : "rgba(255,255,255,0.12)"; (e.currentTarget as HTMLElement).style.boxShadow = ""; }}>
                    <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg,#1a1a2e,#16213e)", opacity: w.minimized ? 0.45 : 1 }}>
                      {aicon(w.appId, 52)}
                    </div>
                    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "rgba(0,0,0,0.75)", padding: "5px 10px 6px", display: "flex", alignItems: "center", justifyContent: "space-between", backdropFilter: "blur(4px)" }}>
                      <span style={{ color: "white", fontSize: 11, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {w.minimized ? "⬛ " : ""}{title}
                      </span>
                      <button
                        onClick={(e) => { e.stopPropagation(); closeWin(w.id); blip("click"); setWindows(ws => { const remaining = ws.filter(x => x.id !== w.id); if (remaining.length === 0) setTaskViewOpen(false); return remaining; }); }}
        ...
        <div className="fixed bottom-10 left-2 z-[9000] flex flex-col rounded-xl overflow-hidden"
          ...
          <div className="flex items-center justify-between px-4 py-3" style={{ borderTop: "1px solid var(--border)", background: "var(--glass)", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.15)" }}>
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl flex items-center justify-center text-lg overflow-hidden flex-shrink-0" style={{ background: avatarBg }}>
                {currentAvatar?.startsWith("data:") ? <img src={currentAvatar} alt="" className="w-full h-full object-cover" /> : (currentAvatar || "👤")}
              </div>
      ...
      {isP4 ? (
        ...
        <div className="fixed bottom-0 left-0 right-0 flex items-center z-[8000]"
          ...
          {/* Start button */}
          {(() => {
            const aC1 = theme.accentC ?? 0.22;
            const aH1 = theme.accentH;
            const isWP = aC1 < 0.05 && theme.accentL !== undefined && theme.accentL > 0.5;
            const isBP = aC1 < 0.05 && theme.accentL !== undefined && theme.accentL <= 0.5;
            const btnBg = isWP
              ? (startOpen ? "#888" : "linear-gradient(180deg,#bbb,#999)")
              : isBP
              ? (startOpen ? "#111" : "linear-gradient(180deg,#333,#111)")
              : startOpen ? `oklch(0.30 ${aC1} ${aH1})` : `linear-gradient(180deg, oklch(0.52 ${Math.min(aC1,0.22)} ${aH1}), oklch(0.36 ${aC1} ${aH1}))`;
            const btnBorder = isWP ? "1px solid #777" : isBP ? "1px solid #000" : `1px solid oklch(0.26 ${aC1} ${aH1})`;
            return (
            <button onClick={(e) => { e.stopPropagation(); blip("click"); setStartOpen(!startOpen); setShowCalendar(false); }}
              title="Start"
              style={{ height: "100%", padding: "0 14px", background: btnBg, border: "none", borderRight: btnBorder, cursor: "pointer", fontWeight: "bold", fontSize: 13, color: "#fff", display: "flex", alignItems: "center", gap: 6, flexShrink: 0, textShadow: "0 1px 2px rgba(0,0,0,0.6)" }}>
              <PueiLogoSvg size={18} />
              Start
            </button>
            );
